...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const totalRevenue = bookings.reduce((sum, booking) => sum + Number.parseInt(booking.amount.replace(/[^0-9]/g, "")), 0)
    const newStats = [
                ...
                <Card key={stat.title} className="overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    <div className="text-2xl font-bold">
                      {stat.title === "Revenue" ? `${stat.value.toLocaleString()} DA` : stat.value.toLocaleString()}
                    </div>
                  </CardContent>
                </Card>
              ))}
